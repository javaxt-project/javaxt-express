...
public class Documentation {
   ...
   */
    private javaxt.io.File getJavaDoc(javaxt.io.Directory dir){
      ...
      //Custom file filter (improves performance on windows when using mounted network drives and UNC paths)
        String lastFile = null;
        for (String fileName : dir.toFile().list()){
            if (fileName.toLowerCase().startsWith(jar.toLowerCase()) && fileName.toLowerCase().endsWith(".zip")){
                lastFile = fileName;
            }
        }
        if (lastFile==null) return null;        
        javaxt.io.File javadoc = new javaxt.io.File(dir, lastFile.substring(0, lastFile.length()-4) + ".xml");
        //System.out.println(javadoc);

      //If the javadoc doesn't exist, extract it from the zip file associated with latest release
        if (!javadoc.exists()){
            javaxt.io.File zipFile = new javaxt.io.File(dir, lastFile);
            try {
